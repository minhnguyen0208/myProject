...
function knobInit(knobId) {
	// Create knob element, 380 x 380 px in size.
	var knob = pureknob.createKnob(380, 380);

	// Create element node.
	var node = knob.node();

	// Add it to the DOM.
	var elem = document.getElementById(knobId);
	elem.appendChild(node);

	// Set properties.
	knob.setProperty('angleStart', -0.75 * Math.PI);
	knob.setProperty('angleEnd', 0.75 * Math.PI);
	knob.setProperty('colorFG', '#343a40');
	knob.setProperty('trackWidth', 0.2);
	knob.setProperty('valMin', 0);
	knob.setProperty('valMax', 100);
	knob.setProperty('readonly', false);
	knob.setProperty('colorBG', '#e9ecef');

	// Set initial value.
	// knob.setValue(loadTempData(myFunction)) ;
